...
class Screen:
    ...
    def __init__(self):
        ...
        hvh = Button(self.screen, self.sokoban, 775, 175, 'HvH')
        hvr = Button(self.screen, self.sokoban, 775, 275, 'HvR')
        rvr = Button(self.screen, self.sokoban, 775, 375, 'RvR')
    ...
    def init_player_styles(self, offset_x=0, offset_y=12) -> None:
        """
        Initialize all of the player styles of Sokoban.
        """
        # Initialize the style images
        # p1 styles
        second_style_offset = (self.sokoban.size // 2 + 1) * Tile.width
        ...
        p1_right = StyleCard(screen=self.screen,
                             ...
                             offset_y=offset_y)
        # p2 styles
        p2_y_offset = ((self.SCREEN_HEIGHT - self.sokoban.size *
                        Tile.height) // 2 + self.sokoban.size * Tile.height)
    ...
    def update_styles(self):
        ...
        styles = self.sokoban.get_styles()
        p1_styles = []
        p2_styles = []
        fifth_style = ''
    ...
    def reset(self) -> None:
        ...
        """
        self.sokoban = SokobanGame(size=5)
        self.tiles = []
        self.dest_tiles = []
        self.tile_origin = None
        self.tile_dest = None
    ...
    def check_winner(self) -> None:
        ...
        """
        if self.sokoban.get_winner() is not None:
    ...
    def move_ai(self) -> None:
        ...
        """
        if isinstance(self.sokoban.whose_turn, PlayerRandom):
            ...
            move = self.sokoban.whose_turn.get_turn()
            if move is not None:
                self.sokoban.move(turn.row_o, move.col_o,
                                  move.row_d, move.col_d, move.style_name)
                self.update_styles()
                self.reset_clicks()
            self.check_winner()
    ...
    def move(self) -> None:
        ...
        if self.sokoban.move(row_o, col_o, row_d, col_d, style_name):
            self.update_styles()
            self.reset_clicks()
    ...
    def set_op(self, game_mode: int) -> None:
        ...
        curr_turn = self.sokoban.whose_turn.player_id
        if game_mode == 0 or game_mode == 2:
            # Set both players to PlayerRandom or a regular Player depending on the game mode.
            self.sokoban.player1 = Player(self.sokoban.player1.player_id) if game_mode == 0 else PlayerRandom(
                self.sokoban.player1.player_id)
            self.sokoban.player2 = Player(self.sokoban.player2.player_id) if game_mode == 0 else PlayerRandom(
                self.sokoban.player2.player_id)
            self.sokoban.player1.set_sokoban(self.sokoban)
            self.sokoban.player2.set_sokoban(self.sokoban)
            if curr_turn == self.sokoban.player1.player_id:
                self.sokoban.whose_turn = self.sokoban.player1
            else:
                self.sokoban.whose_turn = self.sokoban.player2
            return

        # Set the other player to PlayerRandom
        op = self.sokoban.other_player(self.sokoban.whose_turn)
        if not op:
            return
        player = PlayerRandom(op.player_id)
        player.set_sokoban(self.sokoban)
        if self.sokoban.whose_turn == self.sokoban.player1:
            self.sokoban.player2 = player
        else:
            self.sokoban.player1 = player
    ...
    def update_dest_tiles(self) -> None:
        ...
        valid_turns = self.sokoban.whose_turn.get_valid_turns()
        moves = []

        # Check if a style has been chosen and only display those moves.
        if self.chosen_style and self.chosen_style.style_name in valid_turns:
            moves = valid_turns[self.chosen_style.style_name]
        else:
            for style_name in valid_turns:
                moves.extend(valid_turns[style_name])
        # Get all indices of tiles which are valid destinations for the chosen piece.
        highlighted = set()
        for move in moves:
            if (turn.row_o, move.col_o) == (self.tile_origin.row, self.tile_origin.col):
                highlighted.add(self.sokoban.size *
                                move.row_d + move.col_d)
    ...
    def click(self):
        ...
        if clicked:
            ...
            for sty_img in self.player_styles:
                if sty_img.click(mouse_pos) and sty_img.player_id == self.sokoban.whose_turn.player_id:
                    clicked = sty_img
            if clicked:
